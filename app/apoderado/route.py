...
def index():
    ...
    return render_template("apoderado/dashboard.html", apoderado=apoderado, stats_por_alumno=stats)
...
def wizp3():
    apoderado = ctrl.get_apoderado(current_user)
    if request.method == "POST":
        ctrl.update_preferences(apoderado, {
            "notificacion_comprobante": request.form.get("notificacion_comprobante"),
        ...
        merchants_audit.info(
            ...
            apoderado.id, apoderado.nombre, apoderado.usuario.email, len(apoderado.alumnos),
        )
...
def abono():
    ...
    try:
        ...
        flash("Por favor ingrese un monto." if not monto_raw else
              "El monto ingresado no es válido. Por favor ingrese un valor numérico.", "danger")
    ...
    merchants_audit.info(
        ...
        nuevo_abono.codigo, nuevo_abono.apoderado.id,
        nuevo_abono.apoderado.usuario.email, nuevo_abono.monto, nuevo_abono.forma_pago,
    )
    ...
    if forma_pago == "cafeteria":
        ...
        session = flask_merchants.get_client("cafeteria").payments.create_checkout(
            ...
            metadata={"abono_codigo": nuevo_abono.codigo, "apoderado_id": str(nuevo_abono.apoderado.id)},
        )
...
def ordenweb():
    ...
    return jsonify({"status": "OK", "redirect_url": url_for("apoderado_cliente.pago_orden", orden=nueva_orden)})
...
def pago_orden(orden):
    ...
    if pedido:
        ...
        alumnos_by_id = (
            {a.id: a for a in db.session.execute(
                db.select(Alumno).filter(Alumno.id.in_(all_alumno_ids))
            ).scalars().all()}
            if all_alumno_ids else {}
        )
        ...
        if request.method == "POST":
            ...
            send_notificacion_pedido_pendiente.delay({
                ...
                "items": [
                    {
                        "descripcion": i["detalle_menu"].descripcion if i.get("detalle_menu") else i["menu"],
                        "fecha": i["fecha"],
                        "nota": i.get("nota", ""),
                        "precio": int(i["detalle_menu"].precio) if i.get("detalle_menu") else 0,
                        "alumnos_str": ", ".join(
